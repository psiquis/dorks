        ...
        for subdomain in sorted(self.subdomains):
            ...
            for dork_info in dorks:
                ...
                if self.offline or not REQUESTS_AVAILABLE:
                    ...
                    # Progress update cada 10 búsquedas
                    if counter % 10 == 0:
                        progress = (counter / total_combinations) * 100
                        print(f"  {Colors.OKBLUE}[{counter}/{total_combinations}] ({progress:.1f}%) | Hits: {hits_found} | CAPTCHAs: {captcha_encountered}{Colors.ENDC}")

                    # Delay dinámico: más largo si se han encontrado CAPTCHAs
                    if captcha_encountered > 0:
                        delay = random.uniform(6, 12)  # Delays más largos
                    else:
                        delay = random.uniform(4, 8)  # Delays normales
